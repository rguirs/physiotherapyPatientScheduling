...
#now we set the instances parameters
...
namesDaysOfTheWeek = ["MON", "TUE", "WED", "THU", "FRI"] if LANGUAGE == "en" else ["SEG", "TER", "QUA", "QUI", "SEX"]

#creating sheduling examples
if schedule:
    for i in range(len(instances)):
        ...
        print("sheet sysEsc")
        columns_sysEsc = ["ID", "Res", "Phy", "SD00", "SH00", "SD01", "SH01", "SD02", "SH02", "SD03", "SH03", "SD04", "SH04", "SD05", "SH05", "SD06", "SH06", "SD07", "SH07", "SD08", "SH08", "SD09", "SH09", "FD00", "FH00", "FD01", "FH01"]
        df_sysEsc = pd.DataFrame(columns = columns_sysEsc)
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_sysEsc.to_excel(writer, sheet_name=SHEET_SYSESC, index=False)
        
        ##generate sheet "DefSlot"
        print("sheet defSlot")
        columns_defSlot = [FIELD_DEFSLOT_NAME, " ", FIELD_DEFSLOT_START, FIELD_DEFSLOT_END]
        df_defSlot = pd.DataFrame(columns = columns_defSlot)
        for j in range(numberOfSlots[i]):
            df_defSlot.loc[slot_options[j]["Name"], FIELD_DEFSLOT_NAME] = slot_options[j]["Name"]
            df_defSlot.loc[slot_options[j]["Name"], FIELD_DEFSLOT_START] = slot_options[j]["Start"]
            df_defSlot.loc[slot_options[j]["Name"], FIELD_DEFSLOT_END] = slot_options[j]["End"]
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_defSlot.to_excel(writer, sheet_name=SHEET_DEFSLOT, index=False)
            
        ##generate sheet "Pacientes"
        print("sheet pacientes")
        columns_patients = [FIELD_PATIENTS_ID, " ", FIELD_PATIENTS_NAME] + [f"{SESSION_NAME_FOR_USER} {sessions_userNames[j]}" for j in range(10)] + [f"{FOLLOW_NAME_FOR_USER} {(90*j)}" for j in range(1,3)]
        df_patients = pd.DataFrame(columns = columns_patients)
        for patient in patients.keys():
            df_patients.loc[patient, FIELD_PATIENTS_ID] = patient
            df_patients.loc[patient, FIELD_PATIENTS_NAME] = patients[patient]["Name"]
        
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_patients.to_excel(writer, sheet_name=SHEET_PATIENTS, index=False)
            
        ##generate sheet "Pessoal"
        print("sheet pessoal")
        columns_pessoal = [FIELD_STAFF_ID, " ", FIELD_STAFF_ROLE, " ", FIELD_STAFF_NAME]
        df_pessoal = pd.DataFrame(columns = columns_pessoal)
        for j in range(len(names_researchers)):
            name = names_researchers[j]
            df_pessoal.loc[simplifyString(name), FIELD_STAFF_ID] = simplifyString(name)
            df_pessoal.loc[simplifyString(name), FIELD_STAFF_ROLE] = ROLES_RESEARCHER
            df_pessoal.loc[simplifyString(name), FIELD_STAFF_NAME] = name
        for j in range(len(names_physio)):
            name = names_physio[j]
            df_pessoal.loc[simplifyString(name), FIELD_STAFF_ID] = simplifyString(name)
            df_pessoal.loc[simplifyString(name), FIELD_STAFF_ROLE] = ROLES_PHYSIO
            df_pessoal.loc[simplifyString(name), FIELD_STAFF_NAME] = name
            
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_pessoal.to_excel(writer, sheet_name=SHEET_STAFF, index=False)
        
        ##cyclical preferences
        print("sheet cyclical")
        df_patientPreferenceCyclic = pd.DataFrame(columns = [FIELD_PATIENTS_ID, FIELD_PATIENTS_NAME])
        j = 1
        for patient in patients.keys():
            k = 4
            df_patientPreferenceCyclic.loc[j, FIELD_PATIENTS_ID] = patient
            df_patientPreferenceCyclic.loc[j, FIELD_PATIENTS_NAME] = patients[patient]["Name"]
            df_patientPreferenceCyclic.loc[j+1, " "] = " "
            
            N_i = []
            
            w = 0
            for weekday in namesDaysOfTheWeek:
        ...
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_patientPreferenceCyclic.to_excel(writer, sheet_name=SHEET_PATIENTSCYCLIC, index=False)
        
        df_staffPreferenceCyclic = pd.DataFrame(columns = [FIELD_STAFF_ID, FIELD_STAFF_NAME])
        j = 1
        ...
        for name in names_researchers:
            if not isFirst:
                ...
                df_staffPreferenceCyclic.loc[j, FIELD_STAFF_ID] = simplifyString(name)
                df_staffPreferenceCyclic.loc[j, FIELD_STAFF_NAME] = name
                df_staffPreferenceCyclic.loc[j+1, " "] = " "
                slot = random.choice(list(slots.keys()))
                day = random.choice(namesDaysOfTheWeek)
                for w in range(5):
                    if namesDaysOfTheWeek[w] == day:
                        df_staffPreferenceCyclic.loc[j, k] = namesDaysOfTheWeek[w]
                    else:
                        df_staffPreferenceCyclic.loc[j, k] = namesDaysOfTheWeek[w]
        ...
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_staffPreferenceCyclic.to_excel(writer, sheet_name=SHEET_STAFFCYCLIC, index=False)
        
        ##specific preferences
        print("sheet days")
        df_patientPreference = pd.DataFrame(columns = [FIELD_STAFF_ID, FIELD_STAFF_NAME])
        j = 1
        for patient in patients.keys():
            k = 4
            df_patientPreference.loc[j, FIELD_STAFF_ID] = patient
            df_patientPreference.loc[j, FIELD_STAFF_NAME] = patients[patient]["Name"]
        ...
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_patientPreference.to_excel(writer, sheet_name=SHEET_PATIENTSDAYS, index=False)
            
        df_staffPreference = pd.DataFrame(columns = [FIELD_PATIENTS_ID, FIELD_PATIENTS_NAME])
        j = 1
        for name in all_staffNames:
            k = 4
            df_staffPreference.loc[j, FIELD_PATIENTS_ID] = simplifyString(name)
            df_staffPreference.loc[j, FIELD_PATIENTS_NAME] = name
        ...
        with pd.ExcelWriter(file_path, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_staffPreference.to_excel(writer, sheet_name=SHEET_STAFFDAYS, index=False)
...
if reschedule:
    for i in range(len(instances)):
        ...
        df_patients = pd.read_excel(file_path_reschedule, sheet_name=SHEET_PATIENTS, dtype = str).dropna(how='all')
        df_sysEsc = pd.read_excel(file_path_reschedule, sheet_name=SHEET_SYSESC, dtype = str).dropna(how='all')
        df_patientSlotsDays = pd.read_excel(file_path_reschedule, sheet_name=SHEET_PATIENTSDAYS, dtype = str).dropna(how='all')
        ...
        for j in range(numberOfPacients[i]):
            for k in range(10):
                if random.random() < 1/20:
                    df_patients.loc[j, f"{SESSION_NAME_FOR_USER} {sessions_userNames[k]}"] = ""
        ...
        with pd.ExcelWriter(file_path_reschedule, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_patients.to_excel(writer, sheet_name=SHEET_PATIENTS, index=False)
        with pd.ExcelWriter(file_path_reschedule, mode="a", if_sheet_exists="replace", engine="openpyxl") as writer:
            df_patientSlotsDays.to_excel(writer, sheet_name=SHEET_PATIENTSDAYS, index=False)