...
def greedy(initialDay, planningHorizon, slots, staff, patients, N_i, N_pf, schedule):
    ...
    #create two lists with researchers and physio separeted
    all_researchers = []
    all_physio = []
...
def scheduleFollow(N_pf, slots, planningHorizon, patient, researcher, schedule, followUpNum, E_researcher, isReeschedule = False):
    s = False
    ...
    for i in range(1, 15):
        ...
        nextOption = bestDay - timedelta(days=i)
        triesDays.append(nextOption)
    if isReeschedule: #extends the range to a wider number of days if reescheduleing is happening
...
def scheduleSession(N_pf, slots, planningHorizon, patient, researcher, physio, initialDay, schedule, sessionNum, E_researcher, E_physio, isReeschedule = False):
